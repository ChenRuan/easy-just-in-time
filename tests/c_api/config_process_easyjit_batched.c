/*
 * config_process_easyjit_batched.c
 *
 * Variant of config_process_easyjit.c that preserves the same warm-up path
 * but changes steady-state dispatch from per-iteration key switching to
 * contiguous batches per key.
 */
...
int main(int argc, char **argv) {
    ...
    printf("============================================================\n");
    printf("  EasyJIT C Snapshot + Raw Pointer Batched Example\n");
    ...
    start = clock();
    {
        const int base_iters = LOOP_COUNT / CONFIG_MAX;
        const int remainder = LOOP_COUNT % CONFIG_MAX;
        int global_iter = 0;
        int key_index;

        for (key_index = 0; key_index < CONFIG_MAX; ++key_index) {
            const int iters = base_iters + (key_index < remainder ? 1 : 0);
            int iter;

            for (iter = 0; iter < iters; ++iter, ++global_iter) {
                const int group_index = global_iter % GROUP_MAX;
                update_config(key_index, group_index);
                sum += fn_ptrs[key_index]();
            }
        }
    }
    end = clock();
